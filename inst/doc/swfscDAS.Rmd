...
One other processing note is that the 'DateTime' column in the `das_read` output has a time zone of "". This means that when this 'DateTime' column is printed (or any other DateTime object without a timezone), RStudio will print these values with the local timezone. You can confirm the timezone by running `attr(y.read$DateTime, "tzone")`. If you need to do conversions based on time zone, then use the 'OffsetGMT' column and `force_tz` and `with_tz` from the `lubridate` package (but beware of [potential OffsetGMT issues](https://github.com/SWFSC/swfscDAS/issues/12#issuecomment-2704725017)). One reason to do such conversions would be if data was collected in a non-local time zone that causes the daily recorded effort to span more than one day, since then 'OnEffort' values will be incorrect because `das_process` resets the event sequence to off effort at the beginning of a new day.
