...
#'   coerced to class \code{das_df}. This data must be filtered for 'continuous
#'   effort sections; see the Details section below
#' @param ... ignored
#' @param conditions see \code{\link{das_effort}}
#' @param seg.km numeric; target segment length in kilometers
#' @param randpicks.load character, data frame, or \code{NULL}. If character,
#'   must be filename of past randpicks output to load and use (passed to
#'   \code{file} argument of \code{\link[utils:read.table]{read.csv}}). If data
#'   frame, randpicks values will be extracted from the data frame. If
#'   \code{NULL}, new randpicks values will be generated by the function
#' @param distance.method character; see \code{\link{das_effort}}. Default is
#'   \code{NULL} since these distances should have already been calculated
...
#'
#'   This function is intended to be called by \code{\link{das_effort}} when the
#'   "equallength" method is specified. Thus, \code{x} must be filtered for
#'   events (rows) where either the 'OnEffort' column is \code{TRUE} or the
#'   'Event' column is "E"; see \code{\link{das_effort}} for more details. This
#'   function chops each continuous effort section (henceforth 'effort
#'   sections') in \code{x} into modeling segments (henceforth 'segments') of
#'   equal length. Each effort section runs from an "R" event to its
#'   corresponding "E" event. After chopping, \code{\link{das_segdata}} is
...
#'   \itemize{
#'     \item The extra length remaining after chopping is greater than or equal
#'     to half of the target segment length (i.e. \code{>= 0.5*seg.km}): the
#'     extra length is assigned to a random portion of the effort section as its
#'     own segment
#'     (\href{https://github.com/swfsc/swfscDAS/blob/master/inst/DAS_chop_equallength_figures.pdf}{see
#'     Fig. 1a}) \item The extra length remaining after chopping is less than
#'     half of the target segment length (i.e. \code{< 0.5*seg.km}): the extra
#'     length is added to one of the (randomly selected) equal-length segments
#'     (\href{https://github.com/swfsc/swfscDAS/blob/master/inst/DAS_chop_equallength_figures.pdf}{see
#'     Fig. 1b}) \item The length of the effort section is less than or equal to
#'     the target segment length: the entire segment becomes a segment
#'     (\href{https://github.com/swfsc/swfscDAS/blob/master/inst/DAS_chop_equallength_figures.pdf}{see
#'     Fig. 1c}) \item The length of the effort section is zero: a segment of
#'     length zero. If there are more than two events (the "B"/R" and "E"
#'     events), the function throws a warning
#'   }
#'
#'   Therefore, the length of each segment is constrained to be between one half
#'   and one and one half of \code{seg.km} (i.e. \code{0.5*seg.km <=} segment
#'   length \code{>=1.5*seg.km}), and the central tendency is approximately
...
#'   \code{randpicks.load} argument to recreate the same effort segments from
...
#' \itemize{
#'   \item \code{x}, with columns added for the corresponding unique segment
#'   code and number \item segdata: data frame with one row for each segment,
#'   and columns with relevant data (see \code{\link{das_effort}} for specifics)
#'   \item randpicks: data frame with record of length allocations (see Details
#'   section above)
#' }
#'
#' @export
...
#' @name das_chop_equallength
#' @export
das_chop_equallength.das_df <- function(x,
                                        conditions,
                                        seg.km,
                                        randpicks.load = NULL,
